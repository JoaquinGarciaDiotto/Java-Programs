...
public class DiccionarioHashAbierto<K,V> implements Dictionary<K,V>{
	...
	public Iterable<V> eliminarTodas(K key) throws InvalidKeyException{
		...
		PositionList<V> iterable = new ListaDE<V>();
		for(PositionList<Entrada<K, V>> listas : DHA) {
			Position<Entrada<K, V>> entrada;
			try {
				entrada = (listas.isEmpty())? null : listas.first();
				while(entrada!=null)
					if(entrada.element().getKey().equals(key)) {
						iterable.addLast(entrada.element().getValue());
						if(entrada==listas.last()) {
							listas.remove(entrada);
							entrada = null;
						}
						else {
							entrada = listas.next(entrada);
							listas.remove(listas.prev(entrada));
						}
					}
					else
						entrada = (entrada==listas.last())? null : listas.next(entrada);
			} catch (EmptyListException | InvalidPositionException | BoundaryViolationException e) {e.printStackTrace();}
		}
		return iterable;
	}
